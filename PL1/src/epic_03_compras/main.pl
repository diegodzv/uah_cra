...
banner :-
    nl,
    write('========================================================'), nl,
    write(' MONOPOLY PROLOG - EPICA 3: COMPRA AUTOMATICA DE ACTIVOS '), nl,
    write('========================================================'), nl,
    nl,
...
run_scenario(Nombre) :-
    ...
    mostrar_primeras_compras(Eventos, 10).
...
mostrar_primeras_compras([evento_turno(_, _, _, _, _, _, _, _, _, _, _, _, _, no, _, _, _, _, _)|R], N) :-
    mostrar_primeras_compras(R, N).
mostrar_primeras_compras([evento_turno(_, _, _, _, _, _, _, _, _, _, _, _, _, si, evento_compra(Turno, Jugador, Pos, Nombre, Tipo, Color, Precio, DinAntes, DinDespues), _, _, _, _)|R], N) :-
...
export_scenario_csv(Nombre) :-
    ...
    simulacion:simular_partida(PartidaInicial, PartidaFinal, Eventos),
    metricas:resumen_simulacion(PartidaFinal, Eventos, Resumen),
    construir_rutas_csv(Nombre, RutaEventos, RutaComprasTurno, RutaComprasAcum, RutaDineroTurno, RutaPropsTurno, RutaPropsFinales, RutaResumen, RutaOrdenCompras),
