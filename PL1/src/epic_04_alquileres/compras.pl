% =========================
% compras.pl
% Epica 4: logica de compra automatica
% =========================
...
aplicar_compra_si_procede(Turno, comprar_si_tiene_fondos, JugadorIn, casilla(Pos, Tipo, Nombre, Precio, Color), BancoIn,
    ...
    JugadorIn = jugador(NombreJugador, PosJug, DineroAntes, Props, EnCarcel),
    ( banco_posee_activo(BancoIn, Nombre, Activo) ->
        ( DineroAntes >= Precio ->
            ...
            JugadorOut = jugador(NombreJugador, PosJug, DineroDespues, Props2, EnCarcel),
            BancoOut = BancoTmp,
            CompraRealizada = si,
...
banco_posee_activo(banco(_, Activos, _, _), Nombre, Activo) :-
    member(Activo, Activos),
    Activo = activo(Nombre, _, _, _, _).

quitar_activo_banco(banco(Dinero, Activos, Casas, Hoteles), Nombre, banco(Dinero, Activos2, Casas, Hoteles)) :-
