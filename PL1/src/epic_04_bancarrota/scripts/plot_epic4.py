...
def generar_grafica_compras_acumuladas(nombre):
    ...
    filas = leer_csv_dict(ruta)
    series = {
        "compras_acumuladas": [(int(f["turno"]), int(f["compras_acumuladas"])) for f in filas]
    }
    salida = RESULTADOS_DIR / f"{nombre}_grafica_compras_acumuladas.png"
    plot_lineas(series, f"{nombre} - Compras acumuladas", "Turno", "Compras acumuladas", salida)


def generar_grafica_alquileres_por_turno(nombre):
    ruta = RESULTADOS_DIR / f"{nombre}_alquileres_por_turno.csv"
    if not ruta.exists():
        return
    filas = leer_csv_dict(ruta)
    turnos = [int(f["turno"]) for f in filas]
    importes = [float(f["importe_en_turno"]) for f in filas]
    salida = RESULTADOS_DIR / f"{nombre}_grafica_importe_alquileres_por_turno.png"
    plot_barras(turnos, importes, f"{nombre} - Importe de alquileres por turno", "Turno", "Importe", salida, rotacion=0)


def generar_grafica_alquileres_acumulados(nombre):
    ruta = RESULTADOS_DIR / f"{nombre}_alquileres_acumulados.csv"
    if not ruta.exists():
        return
    filas = leer_csv_dict(ruta)
    series = {
        "importe_acumulado_alquileres": [(int(f["turno"]), float(f["importe_acumulado"])) for f in filas]
    }
    salida = RESULTADOS_DIR / f"{nombre}_grafica_importe_alquileres_acumulado.png"
    plot_lineas(series, f"{nombre} - Importe acumulado de alquileres", "Turno", "Importe acumulado", salida)
...
def generar_grafica_dinero_por_turno(nombre):
    ...
    for fila in filas:
        series[fila["jugador"]].append((int(fila["turno"]), float(fila["dinero_final_turno"])))
...
def generar_grafica_balance_alquileres(nombre):
    ruta = RESULTADOS_DIR / f"{nombre}_balance_alquileres_por_jugador.csv"
    if not ruta.exists():
        return
    filas = leer_csv_dict(ruta)
    jugadores = [f["jugador"] for f in filas]
    balance = [float(f["balance_alquiler"]) for f in filas]
    salida = RESULTADOS_DIR / f"{nombre}_grafica_balance_alquileres.png"
    plot_barras(jugadores, balance, f"{nombre} - Balance neto de alquileres", "Jugador", "Balance", salida)
...
def main():
    ...
    for nombre in escenarios:
        ...
        generar_grafica_alquileres_por_turno(nombre)
        generar_grafica_alquileres_acumulados(nombre)
        ...
        generar_grafica_balance_alquileres(nombre)
        print(f"Graficas generadas para: {nombre}")

    print("Todas las graficas de la epica 4 se han generado correctamente.")
