...
banner :-
    nl,
    write('========================================================'), nl,
    write(' MONOPOLY PROLOG - EPICA 8: MONOPOLY                    '), nl,
    write('========================================================'), nl,
    nl,
...
run_scenario(Nombre) :-
    ...
    write('--- PRIMEROS 10 EVENTOS DE MONOPOLY ---'), nl,
    mostrar_primeros_monopoly(Eventos, 10), nl,
    ...
    mostrar_primeros_parking(Eventos, 10), nl,
...
mostrar_primeros_monopoly(Eventos, N) :-
    include(es_evento_monopoly, Eventos, Filtrados),
    mostrar_eventos_monopoly_filtrados(Filtrados, N).

es_evento_monopoly(E) :-
    arg(5, E, si).

mostrar_eventos_monopoly_filtrados([], _) :-
    write(' (ninguno)'), nl.
mostrar_eventos_monopoly_filtrados(_, 0) :- !.
mostrar_eventos_monopoly_filtrados([E|R], N) :-
    arg(6, E, evento_monopoly(Turno, Jugador, Colores, Casas, Coste, DinAntes, DinDespues)),
    write('Turno '), write(Turno),
    write(' | Jugador='), write(Jugador),
    write(' | Colores='), write(Colores),
    write(' | Casas compradas='), write(Casas),
    write(' | Coste='), write(Coste),
    write(' | Dinero '), write(DinAntes), write(' -> '), write(DinDespues), nl,
    N2 is N - 1,
    mostrar_eventos_monopoly_filtrados(R, N2).
...
mostrar_eventos_compra_filtrados([E|R], N) :-
    arg(23, E, evento_compra(Turno, Jugador, Pos, Nombre, Tipo, Color, Precio, DinAntes, DinDespues)),
    write('Turno '), write(Turno),
    write(' | Jugador='), write(Jugador),
    write(' | Pos='), write(Pos),
...
mostrar_eventos_alquiler_filtrados([E|R], N) :-
    arg(25, E, evento_alquiler(Turno, Pagador, Cobrador, Casilla, Tipo, Color, PrecioCasilla, NumCasas, AlquilerTeorico, PagoReal, DinPagAntes, DinPagDespues, DinCobAntes, DinCobDespues)),
    write('Turno '), write(Turno),
...
mostrar_eventos_impuesto_filtrados([E|R], N) :-
    arg(27, E, evento_impuesto(Turno, Jugador, Casilla, Importe, DinAntes, DinDespues, CajaAntes, CajaDespues)),
    write('Turno '), write(Turno),
    write(' | Jugador='), write(Jugador),
    write(' | Casilla='), write(Casilla),
...
mostrar_primeros_parking(Eventos, N) :-
    include(es_evento_parking, Eventos, Filtrados),
    mostrar_eventos_parking_filtrados(Filtrados, N).

es_evento_parking(E) :-
    arg(28, E, si).

mostrar_eventos_parking_filtrados([], _) :-
    write(' (ninguno)'), nl.
mostrar_eventos_parking_filtrados(_, 0) :- !.
mostrar_eventos_parking_filtrados([E|R], N) :-
    arg(29, E, evento_parking(Turno, Jugador, CajaAntes, Premio, CajaDespues, DinAntes, DinDespues)),
    write('Turno '), write(Turno),
    write(' | Jugador='), write(Jugador),
    ...
    write(' | Dinero '), write(DinAntes), write(' -> '), write(DinDespues), nl,
    N2 is N - 1,
    mostrar_eventos_parking_filtrados(R, N2).
...
mostrar_eventos_bancarrota_filtrados([E|R], N) :-
    arg(31, E, evento_bancarrota(Turno, Jugador, Causa, DinAntes, ValorLiquidacion, DinDespues, NumProps, Resultado)),
    write('Turno '), write(Turno),
    write(' | Jugador='), write(Jugador),
...
export_scenario_csv(Nombre) :-
    ...
    simulacion:simular_partida(PartidaInicial, PartidaFinal, Eventos),
    metricas:resumen_simulacion(PartidaFinal, Eventos, Resumen),
    construir_rutas_csv(Nombre, RutaEventos, RutaResumen, RutaCompras, RutaAlquileres, RutaCarcel, RutaImpuestos, RutaParking, RutaBancarrotas, RutaMonopoly),
    ...
    metricas:exportar_orden_monopoly_csv(RutaMonopoly, Eventos),
    nl,
...
construir_rutas_csv(Nombre, RutaEventos, RutaResumen, RutaCompras, RutaAlquileres, RutaCarcel, RutaImpuestos, RutaParking, RutaBancarrotas, RutaMonopoly) :-
    ...
    atomic_list_concat(['resultados/', Nombre, '_orden_monopoly.csv'], RutaMonopoly).
