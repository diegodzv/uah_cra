...
aplicar_alquiler_si_procede(
    ...
    pago_real(DinPagAntes, AlquilerTeorico, PagoReal),
...
pago_real(DineroDisponible, AlquilerTeorico, PagoReal) :-
    ( DineroDisponible >= AlquilerTeorico ->
        PagoReal = AlquilerTeorico
    ;   PagoReal = DineroDisponible
    ).
